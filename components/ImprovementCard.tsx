...
import { AlertCircle, TrendingUp, Sparkles, Copy, Check, X, ChevronDown, ChevronUp } from 'lucide-react';
...
/**
 * CSS Rules applied via Tailwind for the main wrapper:
 * .improvement-card {
 ...
 * }
 * .improvement-card:focus-visible {
...
const ImprovementCard: React.FC<ImprovementCardProps> = ({ improvement, onFix }) => {
  ...
  const contentId = `improvement-content-${improvement.id}`;

  return (
    <div 
      className={`
        bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm overflow-hidden 
        transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 hover:border-indigo-300 dark:hover:border-indigo-500
        focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none group
      `}
      role="button"
      tabIndex={0}
      aria-expanded={isExpanded}
      aria-controls={contentId}
      aria-label={`${improvement.impact} impact improvement: ${improvement.title}. Click to ${isExpanded ? 'collapse' : 'expand'} details.`}
    ...
    >
        ...
        <div className="flex items-center gap-3 shrink-0">
           ...
           {onFix && (
             <button 
               ...
               aria-label={`Ask AI to fix ${improvement.title}`}
               className="
                 ...
                 px-4 py-1.5 rounded-lg text-sm font-bold flex items-center gap-1.5 transition-all shadow-sm active:scale-95
               "
             >
      ...
      {isExpanded && (
        <div 
          id={contentId}
          className="px-4 pb-4 pl-4 sm:pl-[4.5rem] animate-in fade-in slide-in-from-top-1 duration-200"
        >
